...
const NewColumnPreference = ({ columnsWithAccess, setOrder, config, currentScreen = "", columns }: any) => {
  ...
  const tableColumnHeaders = columns()?.fields?.filter((item: any) => !["V2UUID"]?.includes(item.field));
  ...
  const handleInitialLoad = () => {
    if (config && config?.list && Object.keys(config?.list)?.length > 1) {
      ...
      config?.list[config?.selected]?.updatedList?.length > 0 && setUpdatedList(config?.list[config?.selected]?.updatedList);
  ...
  const handleAddList = async (e: any) => {
    ...
    const response: any = await onApply({ order: orderList, unChecked: [], updatedList: updatedList }, listName);
    if (response && response.success) { // or check response.statusCode, etc.
      setPreferenceList([...preferenceList, listName]);
      setPreference({
        ...preference, list: {
          ...preference.list, [listName]: {
            order: config?.list[listName]?.order,
            unChecked: config?.list[listName]?.unChecked,
            updatedList: config?.list[listName]?.updatedList?.length > 0 ? config?.list[listName]?.updatedList : []
          }
        }
      });
  ...
  const onApply = (val?: any, addItem: any = "") => {
    ...
    const payload = {
      [currentScreen]: { ...newConfiguration, list: { ...newConfiguration.list, default: { order: [], unChecked: [], updatedList: [] } } },
      ...(currentScreen === "statementList" && { transactionList: userPreference?.transactionList }),
      ...(currentScreen === "transactionList" && { statementList: userPreference?.statementList }),
      dateFormatType: userPreference?.dateFormatType || DATE_FORMATS_OPTIONS.DATE_TIME_AM_PM,
  ...
  return (
    <>
      ...
      <ConfirmationDialogRaw
        ...
        content={t("User_Settings.Do you really want to delete this record? This process can not be undone")}
        handleSubmit={() => {
          setPreferenceList(preferenceList?.filter((item) => item !== confirmationDialog.item));
          onApply();
        }}
      ...
      >
        <DrawerHeader className="drawer-header preference-header">
          ...
          <h3 title={selectedList}>{t(`${selectedList}`)}</h3>
        </DrawerHeader>
              ...
              <form onSubmit={handleAddList}>
                ...
                <div className="form-group">
                  <input
                    ...
                    value={listName}
                    onChange={e => {
                      const exists = preferenceList.some(item => item.toLowerCase() === e.target.value.toLowerCase());
                      if (exists) {
                  ...
                  />
                  {isListNameError && <span className='error-duplicate-list'> {`${listName} ${t('User_Settings.Preference List error message')}`}</span>}
                  <Button type="submit" disabled={isListNameError}>{t("User_Settings.buttons.Add")}</Button>
                </div>
              </form>
            </div>
          </div>
          <GridItem>
            <div className="additional-preference-radio-column">
              <div className="searchContainer additionalFiterData">
                ...
                <FormControl>
                  ...
                  >
                    {preferenceList?.map((item: any, index: any) => (
                      <div key={index} className="list-action-view">
                        <FormControlLabel
                          title={item}
                          value={item}
                          control={<Radio />}
                          label={item.toLowerCase() === 'default' ? t("User_Settings.default") : item}
                        />
                        <div className="act-btns">
                          <button
                            ...
                            disabled={
                              item === selectedList || ["default", "DAS Lite"].includes(item)
                            }
                            className={
                              item === selectedList || ["default", "DAS Lite"].includes(item)
                ...
                <div>
                  <button className="apply-Preference-filter"
                    onClick={() => {
                      const order = updatedList?.filter((item: any) => !unChecked?.includes(item.field))?.map((item: any) => {
                        return item.field;
                      });
                      const newList = selectedList === "default" ? order : ["action", ...order];
                      setOrder(newList);
                      onApply({ order: newList, unChecked, updatedList: updatedList });
                    }}
                  > <Done /> {t('Filter.button.Apply')}</button>
                </div>
              </div>
            </div>
          </GridItem>
